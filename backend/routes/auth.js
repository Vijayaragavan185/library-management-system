...
router.post('/login', async (req, res) => {
  try {
    ...
    // Get user from database with joins to match the normalized structure
    const [users] = await pool.query(
      `SELECT ua.*, an.username, s.name as student_name, s.department, se.email
       FROM account_usernames an
       JOIN user_accounts ua ON an.account_id = ua.account_id
       LEFT JOIN students s ON ua.student_id = s.student_id
...
// Simple login route with minimal authentication
router.post('/simple-login', async (req, res) => {
  try {
    const { username, password } = req.body;
    console.log(`Simple login attempt for ${username} with password ${password}`);
    
    // Get user info without password check - adapted for normalized structure
    const [users] = await pool.query(
      `SELECT ua.*, an.username, s.name as student_name, s.department, se.email
       FROM account_usernames an
       JOIN user_accounts ua ON an.account_id = ua.account_id
...
// Route to check if a user is authenticated and get their role
router.get('/me', async (req, res) => {
  try {
    // Extract token from authorization header
    const authHeader = req.headers.authorization;
    ...
    // Get user info - adapted for normalized structure
    const [users] = await pool.query(
      `SELECT ua.*, an.username, s.name as student_name
       FROM user_accounts ua 
       LEFT JOIN account_usernames an ON ua.account_id = an.account_id
       LEFT JOIN students s ON ua.student_id = s.student_id
       WHERE ua.account_id = ?`,
      [decoded.id]
    );
...
// Enhanced registration route for normalized database structure
router.post('/register', async (req, res) => {
  try {
    const { student_id, name, email, department, phone, username, password } = req.body;
    ...
    // Check if username already exists in the account_usernames table
    const [existingUsernames] = await pool.query(
      'SELECT * FROM account_usernames WHERE username = ?',
      [username]
    );
    ...
    await pool.query(
      ...
      [student_id, name, department, 'pending']
    );
    
    // Store email in student_emails table
    await pool.query(
    ...
    const [accountResult] = await pool.query(
      'INSERT INTO user_accounts (student_id, password_hash, role) VALUES (?, ?, ?)',
      [student_id, passwordHash, 'student']
    );
    
    // Store username in account_usernames table
    await pool.query(
      'INSERT INTO account_usernames (username, account_id) VALUES (?, ?)',
      [username, accountResult.insertId]
    );
